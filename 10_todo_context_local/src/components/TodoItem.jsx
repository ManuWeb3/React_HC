...
function TodoItem({ todo }) {
  ...
  const [todoMsg, setTodoMsg] = useState(todo.todo) // default = empty todoMsg
  ...
  const editTodo = () => {
    ...
    updateTodo(todo.id, { ...todo, todo: todoMsg }) // updated here itself - POWER of Destructuring
  ...
  return (
    <div
      className={`flex border border-black/10 rounded-lg px-3 py-1.5 gap-x-3 shadow-sm shadow-white/50 duration-300  text-black ${
        todo.completed ? 'bg-[#c6e9a7]' : 'bg-[#ccbed7]' // bg-changes = fn(.completed)
      }`}
    >
      <input
        type="checkbox"
      ...
      <input
        ...
        ${todo.completed ? 'line-through' : ''}`} // strikethrough = fn(.completed)
        value={todoMsg}
      ...
      <button
        ...
        onClick={() => {
          if (todo.completed) return // still an active ToDo = editable, else return
        ...
        disabled={todo.completed} // disabled (true) when it's completed else enabled = disabled is false if NOT completed
      >
        {isTodoEditable ? '📁' : '✏️'}
      ...
      <button
        ...
        onClick={() => deleteTodo(todo.id)} // could have made a separate fn.() that calls deleteTodo() within
      >
        ❌
      </button>
    </div>
